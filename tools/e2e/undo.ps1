...
function Wait-Library {
  # A lista so redesenha quando o comando responde, e no runner a thread
  # principal do app pode estar ocupada: uma leitura unica depois de um sleep
  # fixo ja viu "3 -> 3" com o ditado escondido no app. Le ate a condicao valer
  # ou o prazo acabar, e devolve a ultima leitura (a verificacao mostra o valor).
  param([Parameter(Mandatory)][string]$Probe, [Parameter(Mandatory)][scriptblock]$Ok, [int]$Seconds = 8)
  $sw = [Diagnostics.Stopwatch]::StartNew()
  do {
    $s = EvJson 'library.html' $Probe
...
if ($before.Count -eq 0) {
  ...
  $gone = { param($s) $s.rows -eq $ui0.rows - 1 -and $s.toast }
  $back = { param($s) $s.rows -eq $ui0.rows -and -not $s.toast }
  ...
  # O Ctrl+Z so vale com o toast na tela: espera a exclusao aparecer antes.
  EvJson 'library.html' 'JSON.stringify((() => { document.querySelector("#list .dict .btn-danger").click(); return true; })())' | Out-Null
  Wait-Library $count $gone | Out-Null
